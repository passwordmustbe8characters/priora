...
| **TechCabal** | ✅ Live, no key needed | Public RSS feed + an LLM extraction pass per article. Direct fetch gets IP-reputation-blocked from Vercel's production serverless network (403) — confirmed working fine from a residential IP, so it's not a headers/UA issue. Falls back to rss2json.com's public RSS-to-JSON API in that case (just another RSS reader on the same public feed, via a different network path); verified working end-to-end in production. Real signal, not a structured directory — see the dedup caveat below. |
