...
Write the executive summary and market landscape sections as plain, sourced description of what the research actually found — no interpretation or speculation, that is a separate step done afterward.`;
...
Produce:
1. A market landscape synthesis (2-4 sentences) — your read on what the competitive picture actually means for this founder.
...
Write reasoned inference as inference — "this suggests," "a credible gap appears to be" — never as a confirmed fact. If the competitor data is thin, keep sections general rather than forcing specific claims about any one company.`;
...
async function research(freeVerdict: VerdictResponse): Promise<ResearchOutput> {
...
async function synthesize(freeVerdict: VerdictResponse, competitors: ResearchOutput["competitors"]): Promise<SynthesisOutput> {
  const client = getOpenAI();
...
export async function generateDeepReport(freeVerdict: VerdictResponse): Promise<DeepReportContent> {
  const researched = await research(freeVerdict);
  ...
  const synthesized = await synthesize(freeVerdict, researched.competitors);
