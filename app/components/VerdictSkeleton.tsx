/** Mirrors VerdictResults' shape and dark styling so the swap from
 * skeleton → real result doesn't jump around or flash light-on-dark. */
export function VerdictSkeleton() {
  return (
    <div className="flex h-full min-h-0 animate-pulse flex-col">
      <div className="h-3 w-2/5 rounded-full bg-white/10" />
      <div className="mt-3 h-6 w-4/5 rounded-lg bg-white/10 sm:h-7" />

      <div className="mt-4 grid min-h-0 flex-1 auto-rows-min grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {[0, 1, 2, 3, 4, 5].map((i) => (
          <div key={i} className="h-20 rounded-xl bg-white/5" />
        ))}
      </div>
    </div>
  );
}
